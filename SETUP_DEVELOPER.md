...
### reversible列の追加（裏→表出題の可否フラグ）

`Questions`シートに`reversible`列（任意項目）を追加した。裏→表出題（back→front）の対象から除外したい問題（`front`が自然文の設問になっているものなど）に`N`を入れると、出題方向の設定によらず常に表→裏で出題される。空欄は「裏→表OK」のデフォルト扱いなので、既存データはこの列が無くても動作は変わらない（裏→表出題機能自体を使わない限り影響なし）。

移行手順:
1. 対象スプレッドシートの`Questions`シートで、既存のヘッダー行の右隣（`note`列の次）に`reversible`というヘッダーを追加
2. `Code.gs`を最新内容に貼り替えて保存 → 新バージョンでデプロイ
3. 裏→表にすると成立しない問題（自然文の設問など）があれば、該当行の`reversible`セルに`N`を入力

### note列の追加（例文・解説）

`Questions`シートに`note`列（任意項目）を追加した。既存のスプレッドシートを使っている場合、この列がまだ無いと`note`の値を書き込んでも読み込み時に反映されない（ヘッダー行に列名が無いと値が拾われない仕組みのため）。

移行手順:
1. 対象スプレッドシートの`Questions`シートで、既存のヘッダー行の右隣（`category`列の次）に`note`というヘッダーを追加
2. `Code.gs`を最新内容に貼り替えて保存 → 新バージョンでデプロイ
3. 既存の問題に解説を足したい場合は、該当行の`note`セルに直接入力（アプリ側からの編集機能はまだ無い）
